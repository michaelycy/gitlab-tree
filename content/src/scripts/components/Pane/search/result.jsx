    ...
    <>
      {arr.map((len, i) => {
        ...
        return currString.length === 0 ? null : (
          <span key={i.toString()} className={charClass}>
            {currString}
          </span>
        );
      })}
...
function SearchBarResult({
  ...
  const onHandleRedirect = useCallback(() => handleRedirect(index), [index, handleRedirect]);
  const onActiveResult = useCallback(() => setActiveResult(index), [index, setActiveResult]);
  ...
  return (
    <div className={resultClass} onClick={onHandleRedirect} onMouseEnter={onActiveResult}>
      <div className="spantree-search-file">
        <div className="spantree-file-icon">
          <i className={fileIcons.getClassWithColor(fileName, false)} />
        </div>

        <div className="spantree-search-filename">{fileName}</div>
      </div>

      <div className="spantree-search-file-location">
